...
from reading_stats.export.works import export_works_stats
...
def plot_works_score_scatter(
        df: pd.DataFrame,
        works_to_highlight: list = []) -> None:
    ...
    for work_type in ["Novel", "Short Story", "Novella", "Novelette"]:
        ...
        ax.scatter(df_work_type["ReadScore"], df_work_type["GoodreadsScore"],
                   s=10, color=Settings.WORK_TYPE_COLOR_MAPPING[work_type],
        ...
        for work in works_to_highlight:
            ...
            if not df_work.empty:
                ...
                ax.annotate(
                    ...
                    textcoords="data",
                    xytext=(5.1, df_work["GoodreadsScore"].values[0]),
                    ha='left', va="center",
    ...
    fig.savefig(CONFIG["Works"]["FigOutputFile"], dpi=1000)


if __name__ == "__main__":
    df = export_works_stats()
    plot_works_score_scatter(df, WORKS_TO_HIGHLIGHT)
